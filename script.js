...
function openKeyModal() {
  ...
  openModal(
    ...
    "Enter your Groq API Key to synchronize with Luna.",
    "Save Key",
    "var(--primary)",
    () => {
...
async function handleSignUp() {
  ...
  else notify("Account Created. Welcome to the cosmos.");
}
async function handleSignIn() {
  const { data, error } = await sb.auth.signInWithPassword({
    email: v("in-email"),
    password: v("in-password"),
  });
  if (error) notify("Invalid Credentials.");
...
function initApp(u) {
  ...
  refreshHistory();
  renderMsg(
    "The stars have aligned. I am **Luna**, an intelligence that shines. How shall we illuminate the void today?",
    "bot",
  );
}
function confirmSignOut() {
  openModal(
    "Depart",
    "Shall we drift back into the silence of the stars?",
    "Sign Out",
    "var(--danger)",
    () => {
      sb.auth.signOut();
...
async function chat(isRegenerating = false) {
  ...
  if (lowerText.includes("luis loves who")) {
    const customRes = "His Babyyy - SANG!";
...
async function generateChatTitle(userPrompt, apiKey) {
  ...
  // 1. Check for the specific hardcoded case
  // .toLowerCase() and .trim() ensure it works even with extra spaces or caps
  if (userPrompt.trim().toLowerCase() === "luis loves who?") {
    title = "Luis Loves Her";
  } else {
    // 2. Otherwise, proceed with the API call
    try {
      ...
      title = "New Chat"; // Fallback title on error
    }
  }

  // 3. Insert into Supabase and refresh
  try {
    await sb.from("chat_history").insert([
      {
        user_id: user.id,
...
function confirmDel(e, id) {
  ...
  openModal(
    "Clear Memory?",
    "Shall this constellation be forgotten?",
    "Delete",
    "var(--danger)",
    async () => {
      await sb.from("chat_history").delete().eq("chat_id", id);
      if (currentChatId === id) startNewSession();
      refreshHistory();
    },
  );
}
